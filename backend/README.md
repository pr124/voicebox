...
92 endpoints organized by domain. Full interactive documentation available at \`http://localhost:17493/docs\` when the server is running.
...
| OpenAI compatibility | \`/v1/audio/speech\`, \`/v1/models\` | OpenAI-compatible TTS responses |
...
# OpenAI-compatible speech (writes a WAV file)
curl -X POST http://localhost:17493/v1/audio/speech \\
  -H "Content-Type: application/json" \\
  -d '{"model": "tts-1", "input": "Hello from Voicebox", "voice": "alloy"}' \\
  --output speech.wav
...
The compatibility endpoint supports \`tts-1\`, \`tts-1-hd\`, and \`gpt-4o-mini-tts\`. It returns WAV, FLAC, or raw PCM audio. \`tts-1\` accepts OpenAI voice names and Kokoro voice ids. \`tts-1-hd\` uses a cloned Voicebox profile passed as \`voice\`, or the configured default profile. \`gpt-4o-mini-tts\` accepts OpenAI voice names and Qwen CustomVoice speakers.

### ChatGPT connection

Voicebox also includes an MCP server at \`/mcp\`. ChatGPT cannot call a localhost URL directly; expose the MCP endpoint through a [Secure MCP Tunnel](https://developers.openai.com/api/docs/guides/secure-mcp-tunnels) and add it as a custom app using OpenAI's [MCP connection guide](https://developers.openai.com/plugins/deploy/connect-chatgpt). Keep the backend bound to loopback unless a trusted tunnel or authenticated reverse proxy is in front of it.
